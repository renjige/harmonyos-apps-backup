# 积累日记 — 打包 HAP（自动配置 DevEco 自带 JDK）
# 用法：PowerShell cd 到本目录后执行 .\build-hap.ps1
...
Write-Host "`n==> stop old hvigor daemon (避免 spawn java ENOENT)"
...
if ($LASTEXITCODE -eq 0) {
  Write-Host ""
  Write-Host "HAP: entry\build\default\outputs\default\entry-default-signed.hap" -ForegroundColor Green
} else {
  exit $LASTEXITCODE
}
