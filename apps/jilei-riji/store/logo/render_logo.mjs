...
async function main() {
  ...
  const bg = { background: '#2D2D2D' };
  for (const [name, size] of [['foreground.png', 1024], ['background.png', 1024], ['preview-1024.png', 1024], ['agc-216.png', 216]]) {
    await sharp(svg).resize(size, size).flatten(bg).png().toFile(path.join(LOGO_DIR, name));
  }
  const targets = [
