# CHECKLIST（Gate B）· 文波错题析

- requestId: req_1787366806889
- slug: wenbo-cuoti-fenxi
- appName: 文波错题析
- industry: wrong-notebook
...
- [x] AI quickCommands 含行业词（方案 B 问卷，无 LLM 快捷指令）
...
- [x] 无 Python 时使用 agent-svg + PNG 落盘（ensure-app-logo 可 check-only）
- [x] 工程 app_icon / icon / startIcon 已替换默认图
- [x] AppSpec.brand.logo 已登记 customLock
...
- [x] runtime/{tenantId} 栏目种子已落盘并已 deploy
- [x] Admin 改一条 → App 刷新同步
- [x] App Messages 等已对接平台 API（若 featureModules 含 messages）

## B12 详情

- [x] 错题 / 学科 / 方法 / 建议 / 学习方法详情 Hero+摘要+正文+CTA
...
- status: `pass`（DevEco CompileArkTS 请本机打开 `app/` Sync 确认）
- blockers: []
- warnings: [DevEco 真机编译需本机确认]
