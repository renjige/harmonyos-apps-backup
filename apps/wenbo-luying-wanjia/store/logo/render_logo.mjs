/**
 * Rasterize 露营玩家 logo (tent + door negative space).
 */
import fs from 'fs'
...
const MASTER = 1024
const AGC = 216
const BG = [0x16, 0x33, 0x18, 255]
const MARK = [0xf5, 0xf0, 0xe8, 255]
...
function drawMark(size) {
  ...
  fillPoly(
    ...
    size,
    [
      [Math.round(256 * s), Math.round(96 * s)],
      [Math.round(424 * s), Math.round(384 * s)],
      [Math.round(88 * s), Math.round(384 * s)],
    ],
    MARK,
  )
  fillPoly(
    ...
    size,
    [
      [Math.round(256 * s), Math.round(232 * s)],
      [Math.round(312 * s), Math.round(384 * s)],
      [Math.round(200 * s), Math.round(384 * s)],
    ],
    BG,
  )
  return buf
...
console.log('OK rendered 露营玩家 logo PNGs')
