/**
 * Rasterize 职业课堂 symbol → PNG masters (no Python required).
 */
import fs from 'fs';
...
function fillRect(rgb, x0, y0, x1, y1, color, size) {
...
function drawMark(rgb, size) {
  fillPoly(rgb, LEFT_COVER, PAGE, size);
  fillPoly(rgb, RIGHT_COVER, PAGE_R, size);
  fillRect(rgb, 498, 240, 525, 795, SPINE, size);
  fillPoly(rgb, ARROW, ORANGE, size);
}
...
function main() {
  ...
  console.log('OK rendered PNG masters');
}
