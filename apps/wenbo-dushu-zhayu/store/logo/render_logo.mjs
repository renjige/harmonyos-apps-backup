/**
 * Rasterize 读书札语 symbol → PNG masters (no Python required).
 */
import fs from 'fs';
...
function fillPoly(rgb, poly, color, size) {
  ...
  for (let y = 0; y < size; y++) {
    ...
    for (let k = 0; k + 1 < slice.length; k += 2) {
      let x0 = Math.max(0, Math.ceil(slice[k]));
      let x1 = Math.min(size - 1, Math.floor(slice[k + 1]));
      for (let x = x0; x <= x1; x++) {
        const o = (y * size + x) * 3;
        rgb[o] = color[0];
        rgb[o + 1] = color[1];
...
function drawMark(rgb, size) {
  ...
  fillPoly(rgb, BOOKMARK, GOLD, size);
}
