/**
 * Rasterize 自然探索志 logo — ink ground + forest ridge + amber compass.
 */
import fs from 'fs';
...
const FOREST = [0x2d, 0x5a, 0x27, 255];
const CREAM = [0xf5, 0xf0, 0xe8, 255];
const AMBER = [0xc4, 0x9a, 0x6c, 255];
...
function paintMark(buf, w) {
  fill(buf, w, BG);
  const s = w / 1024;
  fillPoly(buf, w, [[128 * s, 720 * s], [320 * s, 400 * s], [448 * s, 560 * s], [576 * s, 360 * s], [768 * s, 520 * s], [896 * s, 720 * s]], FOREST);
  fillPoly(buf, w, [[448 * s, 720 * s], [512 * s, 600 * s], [576 * s, 720 * s]], BG);
  fillPoly(buf, w, [[512 * s, 240 * s], [592 * s, 420 * s], [512 * s, 480 * s], [432 * s, 420 * s]], CREAM);
  fillPoly(buf, w, [[512 * s, 268 * s], [568 * s, 408 * s], [512 * s, 448 * s], [456 * s, 408 * s]], AMBER);
  fillCircle(buf, w, 512 * s, 436 * s, 32 * s, CREAM);
  fillCircle(buf, w, 512 * s, 436 * s, 14 * s, FOREST);
}
...
console.log('Rendered 自然探索志 logo PNGs');
