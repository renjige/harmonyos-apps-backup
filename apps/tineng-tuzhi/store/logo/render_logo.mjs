...
async function main() {
  ...
  const fg = await sharp(svg).resize(1024, 1024).png().toBuffer();
  await sharp(fg).flatten({ background: '#1A2332' }).png().toFile(path.join(LOGO_DIR, 'foreground.png'));
  await sharp({
    create: { width: 1024, height: 1024, channels: 3, background: '#1A2332' },
  }).png().toFile(path.join(LOGO_DIR, 'background.png'));
  await sharp(fg).flatten({ background: '#1A2332' }).png().toFile(path.join(LOGO_DIR, 'preview-1024.png'));
  await sharp(fg).flatten({ background: '#1A2332' }).resize(216, 216).png().toFile(path.join(LOGO_DIR, 'agc-216.png'));
  const targets = [
