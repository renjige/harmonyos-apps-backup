# 球星风采

当前 App 名称：**球星风采**

魏文波 · HarmonyOS NEXT  
体育人物百科工具：发现球星、查看资料、收藏关注、接收赛事智慧提醒。

## 上架图标（华为应用市场请用这个）

- **`store/logo/preview-1024.png`** ← **上架主图标 1024×1024（优先上传这一张）**
- 工程副本：`app/AppScope/resources/base/media/app_icon.png`
- 模块图标：`app/entry/src/main/resources/base/media/icon.png`
- 启动图：`app/entry/src/main/resources/base/media/startIcon.png`
- 分层：`store/logo/foreground.png` + `store/logo/background.png`

## 目录

```
apps/wenbo-qiuxing-fengcai/
├── app/          # 鸿蒙客户端（DevEco 打开此目录）
├── store/        # 上架资料与 Logo
├── docs/         # 产品文档
└── AppSpec.json
```

## 快速启动

1. DevEco Studio 打开 **`app/`** 目录
2. Sync Project → Build → 运行到模拟器或真机
3. 默认游客可浏览球星资料；关注与提醒需登录

## 硬约束摘要

- 首次进入无独立隐私弹窗；协议勾选在登录/注册页
- 禁止「删除业务数据」按钮；设置内提供「清除缓存」
- 新用户登录后关注列表为空；二级详情 + 三级关注/提醒完整
- 浅色/深色模式文字与背景高对比
