/**
 * Rasterize 夏野漫行 logo with Node (no Python). Mountain + sun + path.
 */
import fs from 'fs'
...
const MASTER = 1024
const AGC = 216
const BG = [0x24, 0x35, 0x2b, 255]
const CREAM = [0xf6, 0xf1, 0xe8, 255]
const SUN = [0xe0, 0x7a, 0x3d, 255]
const MIST = [0xa8, 0xb5, 0xa0, 255]
...
function drawMark(size) {
  ...
  fill(buf, size, BG)
  const s = size / 512
  fillCircle(buf, size, Math.round(318 * s), Math.round(168 * s), Math.round(46 * s), SUN)
  fillPoly(
    buf,
    size,
    [
      [Math.round(64 * s), Math.round(368 * s)],
      [Math.round(176 * s), Math.round(214 * s)],
      [Math.round(248 * s), Math.round(286 * s)],
      [Math.round(320 * s), Math.round(176 * s)],
      [Math.round(448 * s), Math.round(368 * s)],
    ],
    CREAM,
  )
  strokePolyline(
    ...
    size,
    [
      [Math.round(118 * s), Math.round(392 * s)],
      [Math.round(168 * s), Math.round(368 * s)],
      [Math.round(198 * s), Math.round(404 * s)],
      [Math.round(248 * s), Math.round(380 * s)],
      [Math.round(298 * s), Math.round(356 * s)],
      [Math.round(338 * s), Math.round(392 * s)],
      [Math.round(394 * s), Math.round(372 * s)],
    ],
    Math.max(6, Math.round(8 * s)),
    MIST,
  )
  return buf
}
...
console.log('OK rendered 夏野漫行 logo PNGs')
