...
`CompileArkTS` 已通过时，`PackageHap` 仍可能因 **PATH 里找不到 `java`** 失败（只设 `JAVA_HOME` 不够）。

1. 将 **`E:\software\DevEco Studio\jbr\bin`** 加入系统 **Path**
2. **完全退出 DevEco** 再打开（旧 hvigor daemon 不会带上新 PATH）
3. 或在本目录执行：`.\build-hap.ps1`（脚本会设置 `JAVA_HOME`、刷新 PATH、停掉旧 daemon 再打包）
