# 问知 — 打包 HAP（自动配置 DevEco 自带 JDK）
# 用法：在 app/ 目录执行 .\build-hap.ps1
...
if ($LASTEXITCODE -eq 0) {
  Write-Host ""
  Write-Host "HAP: entry\build\default\outputs\default\entry-default-unsigned.hap" -ForegroundColor Green
} else {
  exit $LASTEXITCODE
}
